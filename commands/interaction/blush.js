...
module.exports = {
	...
	async execute(interaction) {
		try {
			...
			//const logEmbed2 = new EmbedBuilder().setColor(color).setDescription(`${tgt} hizo sonrojar a **${guilty}**!`).setImage(nekoUrl).setFooter({ text: 'anime: ' + nekoName + ' | K159|MoccaDev' });
			const logEmbed = {
				color: '#0000ff',
				description:`${tgt} hizo sonrojar a **${guilty}**!`,
				image: nekoUrl,
				footer: {
					text: 'anime: ' + nekoName + ' | K159|MoccaDev'
				}
			}
			counter += 1;
